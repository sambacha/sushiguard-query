...
>(
  ...
  options: UseTransactionOptions<TContract, TMethodName> = {},
): UseTransactionResult<TContract, TMethodName> {
    ...
    mutationFn: async (
      ...
      if (setGasLimit) {
        ...
        if (isOverridesObject(lastItem)) {
          ...
          finalCallArgs = [...argsWithoutOverrides, newOverrides] as ContractMethodArgs<
            TContract,
            TMethodName
          >;
        } else {
          finalCallArgs = [...args, { gasLimit }] as ContractMethodArgs<TContract, TMethodName>;
        }
      }
